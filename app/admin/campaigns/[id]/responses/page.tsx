...
export default async function ResponsesPage(props: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const submittedCount = data.assignments.filter((a) => a.submittedAt).length;
  ...
  const exportHref = `/api/admin/campaigns/${id}/export?${params.toString()}`;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2 text-sm">
            {submittedCount === data.assignments.length && data.assignments.length > 0
              ? <span className="badge pill-success">All submitted</span>
              : submittedCount > 0
                ? <span className="badge pill-info">{submittedCount} submitted</span>
                : <span className="badge pill-neutral">No submissions yet</span>}
            <span style={{ color: 'var(--text-secondary)' }}>
              {submittedCount} / {data.assignments.length} of recipients
            </span>
          </div>
        </div>
        <div className="flex gap-2">
          <Link href={`/admin/campaigns/${id}`} className="btn-secondary">← Campaign</Link>
          <Link href={`/admin/campaigns/${id}/report`} className="btn-secondary">Report</Link>
          <Link href={exportHref} className="btn">Export Excel</Link>
        </div>
      </div>

      <FilterRow locations={locations} officeTypes={officeTypes} departments={departments} sp={sp} basePath={`/admin/campaigns/${id}/responses`} />
      ...
      <div className="card overflow-x-auto">
        ...
        <table className="table">
          <thead>
            <tr>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="empCode" label="Emp. Code" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="name" label="Employee" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="group" label="Group" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="status" label="Status" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="submittedAt" label="Submitted" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th><SortLink basePath={`/admin/campaigns/${id}/responses`} field="reminders" label="Reminders" current={{ sort, dir }} extra={{ locationId: arr('locationId'), officeTypeId: arr('officeTypeId'), departmentId: arr('departmentId') }} /></th>
              <th></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--border-subtle)]">
            {pageSlice.map((a) => (
              <tr key={a.id}>
                ...
                <td>
                  {a.submittedAt
                    ? <span className="badge pill-success" title={formatDateTime(a.submittedAt)}>Submitted</span>
                ...
                <td className="">
                  {a.submittedAt && (
                    <Link href={`/admin/campaigns/${id}/responses/${a.employee.id}`} className="text-[color:var(--text-primary)] hover:underline text-sm">View →</Link>
                  )}
                </td>
              </tr>
            ))}
