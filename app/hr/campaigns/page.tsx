...
export default async function HrCampaignsPage(props: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div className="card overflow-x-auto">
      <div className="flex items-end justify-between flex-wrap gap-3 pb-3 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Campaigns</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            {enriched.length} campaign{enriched.length === 1 ? '' : 's'} visible to HR
          </p>
        </div>
        ...
        <table className="table">
          ...
          <tbody className="divide-y divide-[var(--border-subtle)]">
            {pageSlice.map((c) => {
              ...
              return (
                <tr key={c.id}>
                  <td><Link className="hover:underline" href={`/hr/campaigns/${c.id}`}>{c.title}</Link></td>
            ...
            {enriched.length === 0 && <tr><td colSpan={7} className="text-center text-[color:var(--text-muted)] py-6">No campaigns yet.</td></tr>}
          </tbody>
        </table>
        <div className="pt-3">
